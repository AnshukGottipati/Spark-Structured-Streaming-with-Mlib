...
if not os.path.exists(MODEL_PATH):
    ...
    # We'll also use a simple scaling for the hour_of_day (0-23) and minute_of_hour (0-59)
    # to avoid the model treating them as pure linear numbers (optional but good practice
    # for cyclical features in simple regression).
    ...
    feature_cols = ["hour_of_day", "minute_of_hour"]
...
parsed_stream = raw_stream.select(from_json(col("value"), schema).alias("data")).select("data.*") \
    ...
    .withColumn("fare_amount", col("fare_amount").cast(DoubleType())) # Ensure fare_amount is Double
...
feature_cols_inference = ["hour_of_day", "minute_of_hour"]
assembler_inference = VectorAssembler(inputCols=feature_cols_inference, outputCol="features")
...
# The prediction column is named 'prediction' by default for PySpark ML models.
